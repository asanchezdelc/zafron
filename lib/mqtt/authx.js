...
const pubRegex = /^v1\/[a-zA-Z0-9\-]+\/things\/[a-zA-Z0-9]+\/(sys|data|cmd).*$/;
const subRegex = /^v1\/[a-zA-Z0-9\-]+\/things\/[a-zA-Z0-9]+\/(cmd|conf|data).*$/;
const clientSub = /^v1\/[a-zA-Z0-9\-]+\/things\/#$/;
...
function authorizeSubscribe(client, sub, callback) {
  ...
  // clients are only able to publish to the following topics
  // v1/username/things/#
  if (!subRegex.test(topic) && !clientSub.test(topic)) {
