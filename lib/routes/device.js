...
router.post('/', async (req, res) => {
  try {
    ...
    // check if device with serial already exists
    const existing = await Device.findOne({ serial: body.serial });
    if (existing) {
      return res.status(400).send({ error: 'Device with serial already exists' });
    }
...
router.get('/:id', async (req, res) => {
  try {
    const devices = await Device.findOne({ owner: req.user._id, _id: req.params.id }).populate('profile');
    res.send(devices);
  } catch (error) {
    res.status(500).send();
  }
