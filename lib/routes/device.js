...
router.get('/:id/measurements', async (req, res) => {
  try {
    ...
    if (req.query.endDate && req.query.startDate) {
      ...
      totalReadings = await Metric.countDocuments({ 
        "metadata.serial": dev.serial,
        "timestamp": { $gte: startDate, $lte: endDate }
      });
      readings = await Metric.find({ 
...
router.get('/:id/metrics/:channel', async (req, res) => {
  try {
    const dev = await Device.findOne({ owner: req.user._id, _id: req.params.id });
...
router.get('/:id/latest', async (req, res) => {
  try {
   ...
   const readings = await Metric.aggregate(aggregateQuery);
    res.send(readings);
  } catch (error) {
    console.log(error);
    res.status(500).send();
